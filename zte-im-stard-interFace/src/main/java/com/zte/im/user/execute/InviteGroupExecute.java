...
/**
 * 邀请好友加入群，用于已经建立的群添加新成员，添加成功会更新所有成员群组版本groupVer并发送mqtt消息通知所有成员
 * 
...
public class InviteGroupExecute extends AbstractValidatoExecute {
	...
	private final static Logger logger = LoggerFactory.getLogger(InviteGroupExecute.class);
		...
		if (group != null) {
			...
			if (uids.size() > 0) {
				...
				for (User u : users) {
					...
					bean.setOperatorid(operatoruids.substring(0, operatoruids.length() - 1));
					bean.setModeltype("0");
					list.add(bean);
				}
