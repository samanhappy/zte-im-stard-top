 ...
 * 用户登陆
 * 
 * 根据企业id、工号和密码判断用户是否正确，登录成功时返回用户基本信息和数据版本信息
 * 
		...
		if (gid != null && jid != null && !"".equalsIgnoreCase(jid)
			...
			if (user != null && firm != null) {
				...
				if (user.getUsable() != null && !user.getUsable()) {
					return super.getErrorRes("用户已停用", Constant.USER_DISABLED);
				}
				...
				if (upwd.equalsIgnoreCase(pwd)) {
					...
					try
					{
						...
						oper.append("使用设备(IMEI:");
						oper.append(user.getSn());
						oper.append(")登陆成功");
						TwitterServiceImpl.getInstance().saveUserLog(user, "用户登录", oper.toString());
					}
					catch(Exception e)
					{
					}
