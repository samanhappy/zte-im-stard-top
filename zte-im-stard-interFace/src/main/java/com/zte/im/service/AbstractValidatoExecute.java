...
public abstract class AbstractValidatoExecute extends BaseInteExecute {
	...
	private final static Logger logger = LoggerFactory.getLogger(AbstractValidatoExecute.class);

	public Context context;
	public String[] methods = new String[] { "login", "softwareUpdate", "auth"};
	// 做版本控制的接口
	public String[] versionMethods = new String[] { "listApp", "listAppCate", "findGroup", "enterpriseContact",
			"getWorkImg", "findUserByGid" };
	...
	public String doProcess(Context executeUtilBean) {
		...
		if (jsonRequest == null || "".equals(jsonRequest) || "null".equals(jsonRequest)) {
		...
		// 版本控制
		if (Arrays.toString(versionMethods).contains(executeUtilBean.getMethod())) {
			if (!service.isDataUpdated(executeUtilBean.getMethod(), fastj, user)) {
				Res r = new Res();
