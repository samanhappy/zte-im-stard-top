...
public class MyJSON {
	...
	/**
	 * Parses a JSON string and returns a corresponding Java object. The
	 * returned value is either a {@link com.mongodb.DBObject DBObject} (if the
	 * string is a JSON object or array), or a boxed primitive value according
	 * to the following mapping:
	 * <p>
	 * <code>java.lang.Boolean</code> for <code>true</code> or
	 * <code>false</code><br>
	 * <code>java.lang.Integer</code> for integers between Integer.MIN_VALUE and
	 ...
	 * <p>
	 * If the parameter is a string that contains a single-quoted or
	 * double-quoted string, it is returned as an unquoted
	...
	/**
	 * Parses a JSON string and constructs a corresponding Java object by
	 * calling the methods of a {@link org.bson.BSONCallback BSONCallback}
	 * during parsing. If the callback <code>c</code> is null, this method is
	 * equivalent to {@link com.mongodb.JSON#parse(String) parse(String)}.
	 * 
	 * @param s
	 *            the string to parse
	 * @param c
	...
	public void readHex() {
		if (pos < s.length()
				&& ((s.charAt(pos) >= '0' && s.charAt(pos) <= '9')
						|| (s.charAt(pos) >= 'A' && s.charAt(pos) <= 'F') || (s
			...
			if (current == '\\') {
				...
				case 'u': { // decode unicode
					...
					int codePoint = Integer.parseInt(
							s.substring(tempPos, tempPos + 4), 16);
