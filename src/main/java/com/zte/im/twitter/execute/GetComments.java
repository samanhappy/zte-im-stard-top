...
//				Long[] tIdArr = (Long[]) tIds.toArray(new Long[tIds.size()]);
				List<TComment> comments = twitterService.findUserComments(userId, pageBean);
				int count = 0;
...
//			}
	...
	private void dealResData(Map<String, Object> resComment, TComment comment, Long userId) {
		if (null == comment) {
			return;
		}
		TTwitter twitter = TwitterServiceImpl.getInstance().findTwitterDetail(comment.getTwitterId(), userId);
