...
public class PNGTrunk {
	private static Logger logger = LoggerFactory.getLogger(PNGTrunk.class);
	...
	public static int readInt(byte[] nDest, int nPos) {
		return (nDest[nPos] & 0xFF) << 24 | (nDest[(nPos + 1)] & 0xFF) << 16
				| (nDest[(nPos + 2)] & 0xFF) << 8 | nDest[(nPos + 3)] & 0xFF;
	}

	public static void writeCRC(byte[] nData, int nPos) {
		int chunklen = readInt(nData, nPos);
		logger.info("" + chunklen);
